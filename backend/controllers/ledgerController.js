const { Ledger, LedgerType, Member } = require('../models');
...
exports.getLedgersByMemberId = async (req, res) => {
    try {
...
exports.recalculateAllFines = async (req, res) => {
    try {
...
// Pay a ledger
exports.payLedger = async (req, res) => {
    try {
        const { id } = req.params;
        const ledger = await Ledger.findByPk(id);
...
// Get overdue ledgers
exports.getOverdueLedgers = async (req, res) => {
    try {
        const now = new Date();
        
        // Find ledgers that are past their due date but not paid
        const overdueLedgers = await Ledger.findAll({
            where: {
                due_date: {
        ...
        const updatedLedgers = await Promise.all(
            overdueLedgers.map(async ledger => {
                return await ledger.recalculateFine(Ledger.sequelize.models);
            })
        );
